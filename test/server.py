...
class Control(StreamRequestHandler):
	...
	def handle(self):
		...
		while command not in ['quit','exit']:
			...
			# Testa o recebimento das informacoes
			try:
...
def timed (message):
	...
	return "%s | %-8s | %-6d | %-13s | %s" % (now,'FORKED',os.getpid(),'server.py',message)
...
class Server (TCPServer):
	def server_activate (self):
		print >> sys.stderr, timed('server.py listening on %s:%d' % (host,port))
		sys.stderr.flush()
...
while not connected:
	try:
		...
		count += 1
		if count % 1 == 0:
			print >> sys.stderr, timed('server.py still trying to bind to %s:%d' % (host,port))
